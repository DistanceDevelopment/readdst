...
#    ----- | --------------- | --------- | ---------------------------------------------------- | ------------
  ...
  if(engine %in% c("CDS", "MCDS")){
    ...
    stat_tab[stat_tab$Code==4020,][,4] <- "nrow(model$data)/(model_sum$average.p*dht$individuals$summary$CoveredArea[nrow(dht$individuals$summary)])"
    # estimate Nhat = A*n/(2*w*L*P_a) or Nhat = A*n/(a*P_a)
    stat_tab[stat_tab$Code==4030,][,4] <- "dht$individuals$summary$Area[nrow(dht$individuals$summary)]*nrow(model$data)/(model_sum$average.p*dht$individuals$summary$CoveredArea[nrow(dht$individuals$summary)])"
  }
