...
> The authors of this vignette use computers that work under different operating systems.  ER uses a Windows computer, whereas DLM is fond of his Mac.  Consequently, we have different perspectives regarding how to demonstrate the use of `readdst`.  If you are a regular user of `DistWin` under the Windows operating system, you will recognise the directory structure that the _usual_ installation of `DistWin` creates for storing `DistWin` projects.  The examples presented in this vignette were prepared under the assumption `readdst` would be used on a Windows machine, under this directory structure.  If we ever submit this package to CRAN, we will need to ship the `DistWin` example projects with `readdst`, using a directory structure different from that used by `DistWin`.
