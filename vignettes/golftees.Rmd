...
We can see the names of the analyses (as they were given in Distance for Windows) by calling `names()` on the returned object:
...
```{r fig.cap="Plot results from `analysis_1`.", fig.width=10}
par(mfrow=c(1,2))
plot(analysis_1)
```
...
```{r}
test_stats(converted[[4]], project)
```
