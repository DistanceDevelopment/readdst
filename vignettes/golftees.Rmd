...
We can then convert the whole analysis, this just translates the code that Distance uses to fit the model into R code:

```{r}
converted <- convert_project(project)
```

We can run an individual analysis:

```{r}
run_1 <- run_analysis(converted[[1]])
```
